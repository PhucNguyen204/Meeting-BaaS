...
//
// Phase 1 only ships the Meet implementation under ./meet — Teams/Zoom
...
type Provider interface {
	...
	OpenMeetingPage(ctx context.Context, bctx browser.BrowserContext, link, streamingInput string) (browser.Page, error)

	// JoinMeeting walks the bot through the lobby/Ask-to-join flow and
	// blocks until either accepted or rejected.
	JoinMeeting(ctx context.Context, page browser.Page, opts JoinOptions) error

	// FindEndMeeting reports whether the bot has been removed from the
	// active call (host kicked, "you've been removed" UI, etc).
	FindEndMeeting(ctx context.Context, page browser.Page) (bool, error)

	// CloseMeeting tries to gracefully leave the call.
	CloseMeeting(ctx context.Context, page browser.Page) error
}
