...
// session (matching the TS launchPersistentContext('') behaviour).
//
...
// NewPlaywrightDriver constructs a Driver using playwright-go.
//
// log is required (use logger.New + IntoContext to populate it).
func NewPlaywrightDriver(log *zap.Logger) Driver {
...
func (d *playwrightDriver) Launch(ctx context.Context, opts LaunchOptions) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.context != nil {
...
func (d *playwrightDriver) Context() BrowserContext {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.context
}
...
//
// Wires the per-page diagnostic hook automatically — every page created
...
func (d *playwrightDriver) NewPage(ctx context.Context) (Page, error) {
	...
	// Attach diagnostic logger. Caller does not need to manage teardown —
	// playwright-go cleans up handlers when the page closes.
